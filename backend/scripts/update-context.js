...
const filepath = path.join(__dirname, '../CONTEXTO_COMPLETO_SISTEMA.md');
let content = fs.existsSync(filepath) ? fs.readFileSync(filepath, 'utf8') : '';

const header = `# 📋 CONTEXTO COMPLETO DEL SISTEMA - Sol del Amanecer
...
// Vamos a extraer lo que venía después del flujo diario.
let restOfContent = content;
const splitPoint = restOfContent.indexOf('## 🔄 FLUJO DIARIO COMPLETO');
if (splitPoint !== -1) {
    restOfContent = restOfContent.substring(splitPoint);
}
...
const finalContent = header + '\n\n' + restOfContent.split('**FIN DEL DOCUMENTO - CONTEXTO COMPLETO ACTUALIZADO')[0] + footer;

fs.writeFileSync(filepath, finalContent, 'utf8');
console.log('✅ Contexto actualizado exponencialmente.');
